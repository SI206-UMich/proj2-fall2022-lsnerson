...
def get_detailed_listing_database(html_file):
    ...
    """
    index_tuple = get_listings_from_search_results(html_file)
    final_list = []
    for value in index_tuple:
        second_half = get_listing_information(value[2])
        final_list.append((value[0], value[1], value[2], second_half[0], second_half[1], second_half[2]))
    return final_list
...
def check_policy_numbers(data):
    ...
    """
    policy = '20\d{2}-00\d{4}STR|STR-000\d{4}'
    list_num = []
    for tup in data:
        string_list = re.findall(policy, tup[3])
        if len(string_list) == 0 and tup[3] != "Pending" and tup[3] != "Exempt":
                list_num.append(tup[2])
    return list_num
...
class TestCases(unittest.TestCase):
    ...
    def test_get_detailed_listing_database(self):
        ...
        self.assertEqual(detailed_database[-1][0], 'Guest suite in Mission District')
        self.assertEqual(detailed_database[-1][1], 238)
        self.assertEqual(detailed_database[-1][2], '32871760')
        self.assertEqual(detailed_database[-1][3], 'STR-0004707')
        self.assertEqual(detailed_database[-1][4], 'Entire Room')
        self.assertEqual(detailed_database[-1][5], 1)
